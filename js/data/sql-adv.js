window.QUIZ_BANK.sql.push(
  ...
  { diff: 'normal', type: 'task', q: '고객별 주문 현황을 집계하세요.',
    ...
    steps: [
      { hint: '# 1. 주문이 있는 고객만 이름과 주문 금액을 함께 조회 (내부 조인)', answer: 'select c.name, o.amount from customers c join orders o on c.id = o.customer_id', accept: ['select c.name, o.amount from customers c inner join orders o on c.id = o.customer_id', 'select c.name, o.amount from customers c, orders o where c.id = o.customer_id'] },
      { hint: '# 2. 주문이 없는 고객까지 포함해 조회 (왼쪽 외부 조인)', answer: 'select c.name, o.amount from customers c left join orders o on c.id = o.customer_id', accept: ['select c.name, o.amount from customers c left outer join orders o on c.id = o.customer_id'] },
      { hint: '# 3. 고객별 주문 건수를 세어 3건 이상인 고객만 조회', answer: 'select c.name, count(*) from customers c join orders o on c.id = o.customer_id group by c.name having count(*) >= 3', accept: ['select c.name, count(*) from customers c join orders o on c.id = o.customer_id group by c.name having count(*) > 2', 'select c.name, count(o.id) from customers c join orders o on c.id = o.customer_id group by c.name having count(o.id) >= 3'] },
    ],
